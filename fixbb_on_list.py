...
#                                         ' -database {} -in:file:l {} -resfile {} -ex1 -ex2 -use_input_sc' \
#                                         ' -scorefile design_score.sc -ignore_zero_occupancy false >>design.log'

FIXBB = ROSETTA_DIR + ROSETTA_VERSION + 'main/source/bin/rosetta_scripts.default.linuxgccrelease -database {}' \
                                        '-in:file:l {} -parser:protocol design.xml -parser:script_vars {} ' \


""""~/Rosetta/main/source/bin/rosetta_scripts.default.linuxgccrelease -database ~/Rosetta/main/database/
-in:file:s input_example.pdb -parser:protocol refine.xml -parser:script_vars res_to_fix=1A,2A,3A,4A
-parser:script_vars pdb_reference=input_example.pdb -parser:script_vars cst_full_path=example.cst
-parser:script_vars cst_value=0.4 @flags -overwrite """
...
def run_fixbb(frags):
    ...
    # fixbb run for dictionary of fragments

    print("running fixbb design")

    os.system(FIXBB.format(ROSETTA_DATABASE, list(frags.keys()), list(frags.values())))


def extract_fragments(orig_seq):

    frag_and_resfile = dict()  # fragments and their resfile will be stored here
    ...
    for pdb, chain, start, end, sequence in zip(pdbs, chains, start_res, end_res, sequences):
        ...
        if is_frag_ok:
            frag_and_resfile[outfile] = create_resfile(ori_seq, chain, start, sequence)

    return frag_and_resfile
...
if __name__ == "__main__":
    ...
    frags_for_design = extract_fragments(original_seq)  # extract fragments and create resfiles

    run_fixbb(frags_for_design)  # run fixbb for all the frags

    not_mutated_frags = list(frags_for_design.keys())  # remove the original frags
    for i in range(len(not_mutated_frags)):
        os.remove(not_mutated_frags[i])
