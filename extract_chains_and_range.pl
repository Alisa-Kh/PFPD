...
# modified 2013 by yuval sadan: added -q (quiet) and -a (all chains) options
# original at: /vol/ek/share/labscripts/extract_chains_and_range.pl
#extract_chains.pl
# input: -p pdbin:  pdbfile
#        -c chains: string of chains to extract (orderdependent)
#        -a all chains: extract all chains
#        -o pdbout: output file
#        -n flag to exclude nonAA res: currently PCA, ACE
#	 -q quiet: no log messages to stdout
...
#
my ($opts,$args,$err) = &my_getopts('pcor:thnmqa');  # before : wants arg, after switch (true or false)
@ARGV=@{$args}; my %FLAGS = %{$opts};

$help = $opts->{opt_h};
die "Usage: $0 -p pdbfile -c chains [ -o output (default out) -t (flag for incl of terminus between chains) -r range x_y -n exclude PCA,ACE -m convert mse to met -q (no logs) -a (all chains) -h (flag for help)] " if ($help);

$pdbin  = $FLAGS{opt_p};
die "no pdb input is given (-p pdb)" if ($pdbin eq "");
$chains = $FLAGS{opt_c};
$all_chains = $opts->{opt_a};
die "no chains given (-c chains or -a)" if (!$all_chains && $chains eq "");
$pdbout = $FLAGS{opt_o};
...
print "TER added between chains\n" if (!$quiet && $add_terminus_between_chains);
...
#convert MSE
  if (!$convert_mse){
    if ($line =~ /^HETATM\s*\d+\s+(\w+)\s+MSE\s.\s*(\d+).\s+.*$/){
      $atom= $1;
      $pos = $2;
...
#  print "line $line\n";
  $take_chains{$ch} = 1 if ($all_chains);
...
@chainarray=(1) if ($all_chains);
for ($c=0;$c<@chainarray;$c++){
  for($i=0;$i<@pdbin;$i++){
    print PDBOUT "$pdbin[$i]\n" if ($take_line[$i] == $c+1 || ($all_chains && $take_line[$i]==1));
  }
